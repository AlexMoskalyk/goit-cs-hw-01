...
class TokenType:
    ...
    DIV = "DIV"  # Для ділення
    LPAREN = "LPAREN"  # Для відкриваючих дужок
    RPAREN = "RPAREN"  # Для закриваючих дужок
...
class Parser:
    ...
    def term(self):
        # Обробка множення та ділення з врахуванням їхнього пріоритету
        node = self.factor()
...
class Interpreter:
    ...
    def visit_BinOp(self, node):
        if node.op.type == TokenType.PLUS:
            ...
            if right == 0:
                raise Exception("Помилка: Ділення на нуль.")
