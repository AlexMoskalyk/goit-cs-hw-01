...
class TokenType:
    ...
    DIV = "DIV"      # Для ділення
    LPAREN = "LPAREN"  # Для відкриваючої дужки '('
    RPAREN = "RPAREN"  # Для закриваючої дужки ')'
    EOF = "EOF"
...
class Parser:
    ...
    def factor(self):
        """Фактор це ціле число або вираз у дужках."""
    ...
    def term(self):
        """Терм це фактори, які множаться чи діляться."""
        node = self.factor()
    ...
    def expr(self):
        """Вираз це терми, які додаються або віднімаються."""
        node = self.term()
